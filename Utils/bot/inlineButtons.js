...
var makeRandomButtons = function (flag, data, key, callback) {
  ...
  if(data.length){
    for (var i = 0, len = data.length; i < len; i++){
      ...
      if(i < 2) {
        yesOpt.button[0].push(
          { text: data[i][key], callback_data: flag + (2) + "_" + (i + 1) });
        keyDataInfo.push({id: data[i]._id, text: data[i][key], callback_data: (2) + "_" + (i + 1) });
      } else if(i > 2 && i < 5){
        yesOpt.button[1].push(
          {text: data[i][key], callback_data: flag + (2) + "_" + (i + 1) });
        keyDataInfo.push({id: data[i]._id, text: data[i][key], callback_data: (2) + "_" + (i + 1) });
  ...
  if(data.length){
    for (var i = 0, len = data.length; i < len; i++){
      ...
      if(i < 2) {
        ...
        keyDataInfo.push({id: data[i]._id, text: data[i][key] + " " + data[i][key], callback_data: (2) + "_" + (i + 1) });
      } else if(i > 2 && i < 5){
        yesOpt.button[1].push(
          {text: data[i][key], callback_data: flag + (2) + "_" + (i + 1) });
        keyDataInfo.push({id: data[i]._id, text: data[i][key] + " " + data[i][key], callback_data: (2) + "_" + (i + 1) });
